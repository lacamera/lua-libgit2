...
int lua_git_tree_entry_byid(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument : luagit2_tree,luagit2_oid");
  }

  const luagit2_tree *Tree =
      (luagit2_tree *)luaL_checkudata(L, 1, "luagit2_tree");
  ...
  lua_tree_entry =
      (luagit2_tree_entry *)lua_newuserdata(L, sizeof(*lua_tree_entry));
...
int lua_git_tree_entry_byname(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(L,
                      "expecting exactly 2 argument : luagit2_tree,filename");
  }
...
int lua_git_tree_entry_cmp(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument : luagit2_tree,luagit2_tree");
  }

  const luagit2_tree_entry *Tree_entry1 =
      (luagit2_tree_entry *)luaL_checkudata(L, 1, "luagit2_tree_entry");
  const luagit2_tree_entry *Tree_entry2 =
      (luagit2_tree_entry *)luaL_checkudata(L, 2, "luagit2_tree_entry");

  int result =
      git_tree_entry_cmp(Tree_entry1->tree_entry, Tree_entry2->tree_entry);
...
int lua_git_tree_entry_filemode(lua_State *L) {
  ...
  const luagit2_tree_entry *Tree_entry =
      (luagit2_tree_entry *)luaL_checkudata(L, 1, "luagit2_tree_entry");
...
int lua_git_tree_entry_to_object(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L,
        "expecting exactly 1 argument : luagit2_repository,luagit2_tree_entry");
  }
...
int lua_git_tree_lookup_prefix(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(L,
                      ...
                      "luagit2_repository,luagit2_oid,length");
  }
  ...
  git_tree *local_tree_obj;
  check_error_long(
      git_tree_lookup_prefix(&local_tree_obj, Repo->repo, &(Tree_id->oid),
                             length),
      "Error looking up the tree id in the given repository using its prefix",
      NULL);
