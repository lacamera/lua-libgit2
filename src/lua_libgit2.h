 ...
 * times as luagit2_init() was called.
 *
 * Params required : None
 * Returns : None
 */
...
/* Callable function name : luagit2_SET_SEARCH_PATH( lua_integer Option_Number ,
 lua_string path).
 *
 * Params required : (String) string value of path ,(Integer) Integer option
 number. 1 -> GIT_CONFIG_LEVEL_SYSTEM 2 -> GIT_CONFIG_LEVEL_GLOBAL 3 ->
 GIT_CONFIG_LEVEL_XDG 4 -> GIT_CONFIG_LEVEL_PROGRAMDATA
 * Returns : None.
 */
int lua_GIT_OPT_SET_SEARCH_PATH(lua_State *L);

/* Callable function name : luagit2_GET_CACHED_MEMORY(lua_integer option_number)
 * Get the current bytes in cache and the maximum that would be  allowed in the
 * cache.
 *
 * Params required : (Integer) option number. 1 -> current value, 2 -> allowed
 * value Returns : cache byte value according to selected option number.
 */
 ...
 *
 * Params required : (Integer) integer equivalent of bool. 0 -> disabled , >0 ->
...
/* Callable function name : luagit2_ENABLE_STRICT_OBJECT_CREATION(lua_integer
 * is_enabled) enable or disable creation of objects strictly.
 *
 * Params required : (Integer) integer equivalent of bool. 0 -> disabled , >0 ->
 * enabled. Returns : None.
 */
 ...
 * luagit2_ENABLE_STRICT_SYMBOLIC_REF_CREATION(lua_integer is_enabled) enable or
 * disable creation of symbolic reference strictly.
 *
 * Params required : (Integer) integer equivalent of bool. 0 -> disabled , >0 ->
 * enabled. Returns : None.
 */
...
/* Callable function name : luagit2_ENABLE_STRICT_HASH_VERIFICATION(lua_integer
 * is_enabled) Enable strict verification of object hashsums when reading
 * objects from disk.
 *
 * Params required : (Integer) integer equivalent of bool. 0 -> disabled , >0 ->
 * enabled. Returns : None.
