...
int lua_git_oid_fromstrn(lua_State *L) {
  ...
  check_error_long(git_oid_fromstrn(&local_id, string_value, length),
                   "Error generating oid object from given length of string",
                   NULL);
  obj_id->oid = local_id;
...
int lua_git_oid_cmp(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_oid, luagit2_oid");
  }
...
int lua_git_oid_ncmp(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(
        L, "expecting exactly 3 arguments : luagit2_oid, luagit2_oid, length");
  }
