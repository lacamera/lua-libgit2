...
int lua_git_revwalk_new(lua_State *L) {
  ...
  luagit2_revwalk *lua_revwalk =
      (luagit2_revwalk *)lua_newuserdata(L, sizeof(*lua_revwalk));
...
int lua_git_revwalk_next(lua_State *L) {
  ...
  const luagit2_revwalk *lua_revwalk =
      (luagit2_revwalk *)luaL_checkudata(L, 1, "luagit2_revwalk");
...
int lua_git_revwalk_push(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_revwalk, luagit2_oid");
  }
...
int lua_git_revwalk_push_ref(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_revwalk, (string)ref_name");
  }
