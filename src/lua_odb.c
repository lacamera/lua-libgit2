...
int lua_git_odb_exists(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(L,
                      "expecting exactly 2 argument : luagit2_odb,luagit2_oid");
  }

  const luagit2_odb *lua_odb =
      (luagit2_odb *)luaL_checkudata(L, 1, "luagit2_odb");
...
int lua_git_odb_hash(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument : (string)data,luagit2_otype");
  }

  const char *string_value = luaL_checkstring(L, 1);
  const luagit2_otype *lua_otype =
  ...
  git_oid local_id;
  check_error_long(git_odb_hash(&local_id, (void *)string_value,
                                strlen(string_value), lua_otype->otype),
...
int lua_git_odb_hashfile(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument : (string)path,luagit2_otype");
  }
...
int lua_git_odb_object_free(lua_State *L) {
  ...
  const luagit2_odb_object *lua_odb_obj =
      (luagit2_odb_object *)luaL_checkudata(L, 1, "luagit2_odb_object");
...
int lua_git_odb_object_type(lua_State *L) {
  ...
  luagit2_otype *obj_type =
      (luagit2_otype *)lua_newuserdata(L, sizeof(*obj_type));
...
int lua_git_odb_read(lua_State *L) {
  ...
  luagit2_odb_object *lua_odb_obj =
      (luagit2_odb_object *)lua_newuserdata(L, sizeof(*lua_odb_obj));
  ...
  check_error_long(
      git_odb_read(&local_odb_object, lua_odb->odb, &(obj_id->oid)),
...
int lua_git_odb_write(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(L,
                      ...
                      "luagit2_odb,(string)buffer,luagit2_otype");
  }
  ...
  const luagit2_otype *lua_otype =
      (luagit2_otype *)luaL_checkudata(L, 3, "luagit2_otype");
  luagit2_oid *obj_id = (luagit2_oid *)lua_newuserdata(L, sizeof(*obj_id));
  ...
  git_oid local_id;
  check_error_long(git_odb_write(&local_id, lua_odb->odb, (void *)buffer,
                                 strlen(buffer), lua_otype->otype),
                   "Error generating hash for given file", NULL);
