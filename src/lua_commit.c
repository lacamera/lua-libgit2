...
int lua_git_commit_author(lua_State *L) {
  ...
  const luagit2_commit *lua_commit =
      (luagit2_commit *)luaL_checkudata(L, 1, "luagit2_commit");
...
int lua_git_commit_extract_signature(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting 2 argument(s) : luagit2_repository,luagit2_oid");
  }

  luagit2_repository *Repo =
  ...
  check_error_long(
      git_commit_extract_signature(&local_sign_buf, &local_extra_buf,
                                   Repo->repo, &(Commit_id->oid), NULL),
      "Error extracting signature from given commit id", NULL);

  luagit2_buf *lua_signature_buf =
      (luagit2_buf *)lua_newuserdata(L, sizeof(*lua_signature_buf));
  lua_signature_buf->buf = NULL;
  luaL_newmetatable(L, "luagit2_buf");
  lua_setmetatable(L, -2);
  lua_signature_buf->buf = &local_sign_buf;

  luagit2_buf *lua_extra_buf =
      (luagit2_buf *)lua_newuserdata(L, sizeof(*lua_extra_buf));
...
int lua_git_commit_header_field(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(L,
                      "expecting 2 argument(s) : luagit2_commit,field(string)");
  }

  const luagit2_commit *Commit =
      (luagit2_commit *)luaL_checkudata(L, 1, "luagit2_commit");
...
int lua_git_commit_lookup_prefix(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(
        L,
        "expecting 3 argument(s) : luagit2_repository,luagit2_oid,length(int)");
  }
  ...
  git_commit *local_commit;
  check_error_long(
      git_commit_lookup_prefix(&(local_commit), Repo->repo, &(Commit_id->oid),
                               length),
      "Failed to look up the commit in given repo using length of prefix",
      NULL);
...
int lua_git_commit_message_encoding(lua_State *L) {
  ...
  const char *message_encoding =
      git_commit_message_encoding(lua_commit->commit);
...
int lua_git_commit_nth_gen_ancestor(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(L,
                      "expecting 2 argument(s) : luagit2_commit,nth_gen(int)");
  }
  ...
  lua_ancesstor_commit =
      (luagit2_commit *)lua_newuserdata(L, sizeof(*lua_ancesstor_commit));
  ...
  git_commit *local_commit;
  check_error_long(
      git_commit_nth_gen_ancestor(&(local_commit), lua_commit->commit,
                                  Nth_ancestor_gen),
...
int lua_git_commit_parent(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting 2 argument(s) : luagit2_commit,parent_number(int)");
  }
...
int lua_git_commit_parent_id(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(L,
                      "expecting 2 argument(s) : luagit2_commit,position(int)");
  }
...
int lua_git_commit_create_update_head(lua_State *L) {
  if (lua_gettop(L) != 6) {
    return luaL_error(
        L,
        "expecting 6 argument(s) : luagit2_repository,luagit2_signature(author),luagit2_signature(committer)\
        	commit_message,luagit2_tree,luagit2_commit");
  }

  const luagit2_repository *Repo =
      (luagit2_repository *)luaL_checkudata(L, 1, "luagit2_repository");
  const luagit2_signature *Author_sign =
      (luagit2_signature *)luaL_checkudata(L, 2, "luagit2_signature");
  const luagit2_signature *Commiter_sign =
      (luagit2_signature *)luaL_checkudata(L, 3, "luagit2_signature");
  const char *commit_message = luaL_checkstring(L, 4);
  const luagit2_tree *Tree =
      (luagit2_tree *)luaL_checkudata(L, 5, "luagit2_tree");
  const luagit2_commit *Parent_Commit =
      (luagit2_commit *)luaL_checkudata(L, 6, "luagit2_commit");
  ...
  git_oid local_oid;
  check_error_long(
      git_commit_create_v(&local_oid, Repo->repo, "HEAD", Author_sign->sign,
                          Commiter_sign->sign, NULL, commit_message, Tree->tree,
                          1, Parent_Commit->commit),
...
int lua_git_commit_create_update_none(lua_State *L) {
  ...
  git_oid local_oid;
  check_error_long(
      git_commit_create_v(&local_oid, Repo->repo, NULL, Author_sign->sign,
                          Commiter_sign->sign, NULL, commit_message, Tree->tree,
                          1, Parent_Commit->commit),
...
int lua_git_commit_create_with_signature(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(
        ...
        "luagit2_repository,commit_content(string),commit_sign(string)");
  }
  ...
  git_oid local_oid;
  check_error_long(git_commit_create_with_signature(
                       &local_oid, Repo->repo, commit_content, commit_sign,
...
int lua_git_commit_create_initial(lua_State *L) {
  if (lua_gettop(L) != 5) {
    return luaL_error(
        L,
        "expecting 5 argument(s) : luagit2_repository,luagit2_signature(author),luagit2_signature(committer)\
  ...
  git_oid local_oid;
  check_error_long(git_commit_create_v(&local_oid, Repo->repo, "HEAD",
                                       Author_sign->sign, Commiter_sign->sign,
                                       NULL, commit_message, Tree->tree, 0),
