...
int lua_git_ignore_add_rule(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_repository,(string)rule");
  }
...
int lua_git_ignore_path_is_ignored(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_repository,(string)path");
  }
  ...
  int is_ignored;
  check_error_long(
      git_ignore_path_is_ignored(&is_ignored, Repository->repo, path),
      "Error creating ignore rules for given repo", NULL);
