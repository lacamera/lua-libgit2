...
int lua_git_blob_create_fromdisk(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument(s) : luagit2_repository,path(string)");
  }
...
int lua_git_blob_filtered_content(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(L,
                      "expecting exactly 3 argument(s) : "
                      "luagit2_blob,path(string),check_for_binary_data(int)");
  }

  const luagit2_blob *lua_blob =
      (luagit2_blob *)luaL_checkudata(L, 1, "luagit2_blob");
  ...
  git_buf *local_buf;
  check_error_long(git_blob_filtered_content(local_buf, lua_blob->blob, as_path,
...
int lua_git_blob_lookup(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument(s) : luagit2_repository,luagit2_oid");
  }
...
int lua_git_blob_lookup_prefix(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(L,
                      "expecting exactly 3 argument(s) : "
                      "luagit2_repository,luagit2_oid,length(int)");
  }
