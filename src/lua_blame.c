...
int lua_git_blame_buffer(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(L,
                      ...
                      "luagit2_blame,buffer(string),buffer_length(int)");
  }

  luagit2_blame *lua_blame;
  const luagit2_blame *reference =
      (luagit2_blame *)luaL_checkudata(L, 1, "luagit2_blame");
...
int lua_git_blame_file(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_repository,path(string)");
  }
...
int lua_git_blame_get_hunk_byindex(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_blame,index_num(int)");
  }
...
int lua_git_blame_get_hunk_byline(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_blame,line_num(int)");
  }
