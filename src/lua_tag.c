...
int lua_git_tag_annotation_create(lua_State *L) {
  if (lua_gettop(L) != 5) {
    return luaL_error(
        L,
        "expecting exactly 5 arguments : luagit2_repository,string_name,\
  ...
  const luagit2_object *target_obj =
      (luagit2_object *)luaL_checkudata(L, 3, "luagit2_object");
  const luagit2_signature *tagger =
  ...
  check_error_long(
      git_tag_annotation_create(&(obj_id->oid), Repository->repo, tag_name,
                                target_obj->object, tagger->sign, tag_message),
      "Error creating tag for given object", NULL);
...
int lua_git_tag_create(lua_State *L) {
  if (lua_gettop(L) != 6) {
    return luaL_error(
        L,
        "expecting exactly 6 arguments : luagit2_repository,string_name,\
  ...
  check_error_long(
      git_tag_create(&(obj_id->oid), Repository->repo, tag_name,
                     target_obj->object, tagger->sign, tag_message, force),
      "Error creating tag for given object", NULL);
...
int lua_git_tag_create_frombuffer(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(L,
                      ...
                      "luagit2_repository,string_name,int_force");
  }
  ...
  check_error_long(git_tag_create_frombuffer(&(obj_id->oid), Repository->repo,
                                             buffer, force),
                   "Error creating tag using given buffer", NULL);
...
int lua_git_tag_create_lightweight(lua_State *L) {
  if (lua_gettop(L) != 4) {
    return luaL_error(
        ...
        "luagit2_repository,string_name,luagit2_object,int_force");
  }
  ...
  check_error_long(
      git_tag_create_lightweight(&(obj_id->oid), Repository->repo, tag_name,
                                 target_obj->object, force),
      "Error creating lightweight tag for given object", NULL);
...
int lua_git_tag_delete(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_repository,string_name");
  }
...
int lua_git_tag_list(lua_State *L) {
  ...
  luagit2_strarray *lua_array =
      (luagit2_strarray *)lua_newuserdata(L, sizeof(*lua_array));
...
int lua_git_tag_list_match(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_repository,string_pattern");
  }
...
int lua_git_tag_lookup(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_repository,luagit2_oid");
  }
...
int lua_git_tag_lookup_prefix(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(L,
                      "expecting exactly 3 arguments : "
                      "luagit2_repository,luagit2_oid,int_length");
  }
  ...
  git_tag *local_tag;
  check_error_long(git_tag_lookup_prefix(&local_tag, lua_repo->repo,
                                         &(lua_oid->oid), length),
                   "Error looking up the tag in the given repo using prefix",
                   NULL);
  lua_tag->tag = local_tag;
...
int lua_git_tag_owner(lua_State *L) {
  ...
  const luagit2_tag *lua_tag =
      (luagit2_tag *)luaL_checkudata(L, 1, "luagit2_tag");

  luagit2_repository *lua_repo =
      (luagit2_repository *)lua_newuserdata(L, sizeof(*lua_repo));
  lua_repo->repo = NULL;
...
int lua_git_tag_target(lua_State *L) {
  ...
  luagit2_object *lua_git_obj =
      (luagit2_object *)lua_newuserdata(L, sizeof(*lua_git_obj));
...
int lua_git_tag_target_type(lua_State *L) {
  ...
  luagit2_otype *target_type =
      (luagit2_otype *)lua_newuserdata(L, sizeof(*target_type));
