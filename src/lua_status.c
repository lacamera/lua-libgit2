...
int lua_git_status_list_new(lua_State *L) {
  ...
  luagit2_status_list *lua_status_list =
      (luagit2_status_list *)lua_newuserdata(L, sizeof(*lua_status_list));
  ...
  check_error_long(
      git_status_list_new(&local_status_list, Repository->repo, &status_opt),
...
int lua_git_status_list_free(lua_State *L) {
  ...
  const luagit2_status_list *lua_status_list =
      (luagit2_status_list *)luaL_checkudata(L, 1, "luagit2_status_list");
...
int lua_git_status_byindex(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(L,
                      ...
                      "luagit2_status_list,(number)index_number");
  }
  ...
  luagit2_status_entry *lua_status_entry =
      (luagit2_status_entry *)lua_newuserdata(L, sizeof(*lua_status_entry));
  ...
  lua_status_entry->status_entry =
      git_status_byindex(lua_status_list->status_list, index_number);
