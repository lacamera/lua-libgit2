...
int lua_git_libgit2_features(lua_State *L) {
  ...
  check_error_long(git_libgit2_features(), "Error Accessing libgit2 features",
                   NULL);
...
int lua_GIT_OPT_GET_SEARCH_PATH(lua_State *L) {
  ...
  if ((cfg_lvl = (strcasecmp(option, "system") == 0 ? GIT_CONFIG_LEVEL_SYSTEM
                                                    : 0)) != 0) {
    goto push_val;
  }
  if ((cfg_lvl = (strcasecmp(option, "global") == 0 ? GIT_CONFIG_LEVEL_GLOBAL
                                                    : 0)) != 0) {
    goto push_val;
  }
  if ((cfg_lvl = (strcasecmp(option, "xdg") == 0 ? GIT_CONFIG_LEVEL_XDG : 0)) !=
      0) {
    goto push_val;
  }
  if ((cfg_lvl = (strcasecmp(option, "programdata") == 0
...
push_val : {
  check_error_long(
      git_libgit2_opts(GIT_OPT_GET_SEARCH_PATH, cfg_lvl, &path),
      "error getting search path",
      "Unsupported option, must be one of : system, global, xdg, programdata");
...
int lua_GIT_OPT_SET_SEARCH_PATH(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(L,
                      "expecting exactly 2 argument : search_path_level, path");
  }
...
set_val : {
  check_error_long(
      git_libgit2_opts(GIT_OPT_SET_SEARCH_PATH, cfg_lvl, path),
      "error setting search path",
      "Unsupported option, must be one of : system, global, xdg, programdata");
  return 1;
}
...
int lua_GIT_OPT_SET_CACHE_OBJECT_LIMIT(lua_State *L) {
  ...
  check_error_long(
      git_libgit2_opts(GIT_OPT_SET_CACHE_OBJECT_LIMIT, obj_type->otype, size),
