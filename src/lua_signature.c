...
int lua_git_signature_default(lua_State *L) {
  ...
  check_error_long(
      git_signature_default(&local_sign_obj, Repo->repo),
...
int lua_git_signature_dup(lua_State *L) {
  ...
  luagit2_signature *lua_sign;
  const luagit2_signature *source_sign =
      (luagit2_signature *)luaL_checkudata(L, 1, "luagit2_signature");
...
int lua_git_signature_now(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument : name_string,email_string");
  }
