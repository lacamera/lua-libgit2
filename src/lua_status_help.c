...
int lua_git_status_type(lua_State *L) {
  ...
  const luagit2_status_entry *lua_status_entry =
      (luagit2_status_entry *)luaL_checkudata(L, 1, "luagit2_status_entry");
...
int lua_get_status_wkdir_file_paths(lua_State *L) {
  ...
  const char *old_file_path =
      (lua_status_entry->status_entry)->index_to_workdir->old_file.path
          ? (lua_status_entry->status_entry)->index_to_workdir->old_file.path
          : "NULL";
  const char *new_file_path =
      (lua_status_entry->status_entry)->index_to_workdir->new_file.path
          ? (lua_status_entry->status_entry)->index_to_workdir->new_file.path
          : "NULL";
...
int lua_get_status_index_file_paths(lua_State *L) {
  ...
  const char *old_file_path =
      (lua_status_entry->status_entry)->head_to_index->old_file.path
          ? (lua_status_entry->status_entry)->head_to_index->old_file.path
          : "NULL";
  const char *new_file_path =
      (lua_status_entry->status_entry)->head_to_index->new_file.path
          ? (lua_status_entry->status_entry)->head_to_index->new_file.path
          : "NULL";
