...
int lua_git_revparse(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_repository, (string)spec");
  }
  ...
  luagit2_revspec *lua_revspec =
      (luagit2_revspec *)lua_newuserdata(L, sizeof(*lua_revspec));
...
int lua_git_revparse_single(lua_State *L) {
  ...
  luagit2_object *lua_object =
      (luagit2_object *)lua_newuserdata(L, sizeof(*lua_object));
