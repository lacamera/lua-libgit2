...
int lua_git_object__size(lua_State *L) {
  ...
  const luagit2_otype *Type =
      (luagit2_otype *)luaL_checkudata(L, 1, "luagit2_otype");
...
int lua_git_object_free(lua_State *L) {
  ...
  luagit2_object *lua_object =
      (luagit2_object *)luaL_checkudata(L, 1, "luagit2_object");
...
int lua_git_object_lookup(lua_State *L) {
  ...
  check_error_long(git_object_lookup(&local_obj, Repository->repo,
                                     &(object_id->oid), Type->otype),
...
int lua_git_object_lookup_bypath(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(
        L, "expecting 3 argument(s) : luagit2_object,path,luagit2_otype");
  }

  luagit2_object *lua_object;
  const luagit2_object *treeish =
      (luagit2_object *)luaL_checkudata(L, 1, "luagit2_object");
  ...
  git_object *local_obj;
  check_error_long(
      git_object_lookup_bypath(&local_obj, treeish->object, path, Type->otype),
...
int lua_git_object_lookup_prefix(lua_State *L) {
  if (lua_gettop(L) != 4) {
    return luaL_error(
        ...
        "luagit2_repository,luagit2_oid,length(int),luagit2_otype");
  }
  ...
  const luagit2_otype *Type =
      (luagit2_otype *)luaL_checkudata(L, 4, "luagit2_otype");
  ...
  git_object *local_obj;
  check_error_long(
      git_object_lookup_prefix(&local_obj, Repository->repo, &(object_id->oid),
                               length, Type->otype),
      "Error looking up the object in given repo for given length prefix",
      NULL);
  lua_object->object = local_obj;
