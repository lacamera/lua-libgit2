...
int lua_git_cred_ssh_key_memory_new(lua_State *L) {
  if (lua_gettop(L) != 4) {
    return luaL_error(
        L,
        "expecting 4 argument(s) : username,publickey,privatekey,passphrase");
  }
  ...
  git_cred *out = NULL;
  check_error_long(
      git_cred_ssh_key_memory_new(&out, username, publickey, privatekey,
                                  passphrase),
...
int lua_git_cred_free(lua_State *L) {
  ...
  const luagit2_cred_object *cred_obj =
      (luagit2_cred_object *)luaL_checkudata(L, 1, "luagit2_cred_object");
