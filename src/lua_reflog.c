...
int lua_git_reflog_read(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments : luagit2_repository, (string) name");
  }
  ...
  luagit2_reflog *lua_reflog =
      (luagit2_reflog *)lua_newuserdata(L, sizeof(*lua_reflog));
...
int lua_git_reflog_entrycount(lua_State *L) {
  ...
  const luagit2_reflog *lua_reflog =
      (luagit2_reflog *)luaL_checkudata(L, 1, "luagit2_reflog");
...
int lua_git_reflog_entry_byindex(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 arguments: luagit2_reflog,(number)index");
  }
  ...
  luagit2_reflog_entry *lua_reflog_entry =
      (luagit2_reflog_entry *)lua_newuserdata(L, sizeof(*lua_reflog_entry));
  lua_reflog_entry->reflog_entry = NULL;

  luaL_newmetatable(L, "luagit2_reflog_entry");
  lua_setmetatable(L, -2);

  lua_reflog_entry->reflog_entry =
      git_reflog_entry_byindex(lua_reflog->reflog, index);
...
int lua_git_reflog_entry_committer(lua_State *L) {
  ...
  const luagit2_reflog_entry *lua_reflog_entry =
      (luagit2_reflog_entry *)luaL_checkudata(L, 1, "luagit2_reflog_entry");

  luagit2_signature *lua_sign =
      (luagit2_signature *)lua_newuserdata(L, sizeof(*lua_sign));
  lua_sign->sign = NULL;
...
int lua_git_reflog_entry_message(lua_State *L) {
  ...
  const luagit2_reflog_entry *lua_reflog_entry =
      (luagit2_reflog_entry *)luaL_checkudata(L, 1, "luagit2_reflog_entry");
  const char *message =
      git_reflog_entry_message(lua_reflog_entry->reflog_entry);
...
int lua_git_reflog_drop(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(L,
                      ...
                      "luagit2_reflog,(number)index,(integer)should_rewrite");
  }
...
int lua_git_reflog_append(lua_State *L) {
  if (lua_gettop(L) != 4) {
    return luaL_error(
        ...
        "luagit2_reflog,luagit2_oid,luagit2_signature,(string)message");
  }
  ...
  const luagit2_signature *lua_sign =
      (luagit2_signature *)luaL_checkudata(L, 3, "luagit2_signature");
  const char *message = luaL_checkstring(L, 4);

  check_error_long(git_reflog_append(lua_reflog->reflog, &(lua_oid->oid),
                                     lua_sign->sign, message),
