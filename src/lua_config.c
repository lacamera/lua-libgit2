...
int lua_git_config_delete_entry(lua_State *L) {
  ...
  check_error_long(git_config_delete_entry(parent_cfg->cfg, name),
                   "Unable to delete the entry from given config, entry's name",
                   NULL);
...
int lua_git_config_delete_multivar(lua_State *L) {
  ...
  check_error_long(
      ...
      "Unable to delete the entry from given config , entry's name and regex",
      NULL);
...
int lua_git_config_find_programdata(lua_State *L) {
  ...
  check_error_long(
      ...
      "Unable to find locate the path to the configuration file in ProgramData",
      NULL);
  lua_buf->buf = &local_buf;
...
int lua_git_config_find_system(lua_State *L) {
  ...
  check_error_long(git_config_find_system(&local_buf),
                   "Unable to locate the path to the system configuration file",
                   NULL);
  lua_buf->buf = &local_buf;
...
int lua_git_config_find_xdg(lua_State *L) {
  ...
  check_error_long(git_config_find_xdg(&local_buf),
                   "Unable to Locate the path to the global xdg compatible "
                   "configuration file",
                   NULL);
  lua_buf->buf = &local_buf;
...
int lua_git_config_open_level(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting 2 argument(s) : luagit2_config,luagit2_config_level_t");
  }
  ...
  parent_cfg = (luagit2_config *)luaL_checkudata(L, 1, "luagit2_config");
  luagit2_config_level_t *Level =
      (luagit2_config_level_t *)luaL_checkudata(L, 2, "luagit2_config_level_t");
...
int lua_git_config_set_bool(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(
        L, "expecting 3 argument(s) : luagit2_config,name(string),value(int)");
  }
...
int lua_git_config_set_multivar(lua_State *L) {
  if (lua_gettop(L) != 4) {
    return luaL_error(
        ...
        "luagit2_config,name(string),regex(string),value(string)");
  }
...
int lua_git_config_set_string(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(
        L,
        "expecting 3 argument(s) : luagit2_config,name(string),value(string)");
  }
...
int lua_git_config_free(lua_State *L) {
  ...
  const luagit2_config *lua_cfg =
      (luagit2_config *)luaL_checkudata(L, 1, "luagit2_config");
...
int lua_git_config_add_file_ondisk(lua_State *L) {
  if (lua_gettop(L) != 5) {
    return luaL_error(
        L,
        "expecting 5 argument(s) : luagit2_config,path,luagit2_config_level_t,luagit2_repository\
        	force(int)");
  }

  const luagit2_config *Config_to_write =
      (luagit2_config *)luaL_checkudata(L, 1, "luagit2_config");
  const char *path = luaL_checkstring(L, 2);
  const luagit2_config_level_t *cfg_lvl =
      (luagit2_config_level_t *)luaL_checkudata(L, 3, "luagit2_config_level_t");
  const luagit2_repository *Repository =
      (luagit2_repository *)luaL_checkudata(L, 4, "luagit2_repository");
  const int force = luaL_checkinteger(L, 5);

  check_error_long(
      git_config_add_file_ondisk(Config_to_write->cfg, path, cfg_lvl->level,
                                 Repository->repo, force),
      "Unable to add file ondisk", NULL);
  return 1;
}
...
int lua_git_config_new(lua_State *L) {
  ...
  git_config *local_config;
  check_error_long(git_config_new(&local_config), "Error creating new config",
                   NULL);
  lua_cfg->cfg = local_config;
...
int lua_git_config_iterator_new(lua_State *L) {
  ...
  lua_cfg_itr =
      (luagit2_config_iterator *)lua_newuserdata(L, sizeof(*lua_cfg_itr));
...
int lua_git_config_entry_free(lua_State *L) {
  ...
  luagit2_config_entry *cfg_entry =
      (luagit2_config_entry *)luaL_checkudata(L, 1, "luagit2_config_entry");
...
int lua_git_config_get_entry(lua_State *L) {
  ...
  lua_cfg_entry =
      (luagit2_config_entry *)lua_newuserdata(L, sizeof(*lua_cfg_entry));
  ...
  check_error_long(
      git_config_get_entry(&local_config_entry, parent_cfg->cfg, name),
...
int lua_git_config_next(lua_State *L) {
  ...
  luagit2_config_entry *lua_cfg_entry;
  const luagit2_config_iterator *cfg_itr =
      (luagit2_config_iterator *)luaL_checkudata(L, 1,
  ...
  git_config_entry *local_config_entry;
  check_error_long(
      git_config_next(&local_config_entry, cfg_itr->config_iterator),
...
int lua_git_config_iterator_free(lua_State *L) {
  ...
  luagit2_config_iterator *cfg_itr = (luagit2_config_iterator *)luaL_checkudata(
      L, 1, "luagit2_config_iterator");
