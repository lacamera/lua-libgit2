...
int lua_git_reference_create(lua_State *L) {
  if (lua_gettop(L) != 5) {
    return luaL_error(
        L,
        "expecting exactly 5 argument : luagit2_repository,name,luagit2_oid(target),\
			force(int),log_message");
  }

  luagit2_reference *lua_ref;
  const luagit2_repository *lua_repo =
      (luagit2_repository *)luaL_checkudata(L, 1, "luagit2_repository");
  const char *name = luaL_checkstring(L, 2);
  const luagit2_oid *lua_oid =
      (luagit2_oid *)luaL_checkudata(L, 3, "luagit2_oid");
  ...
  check_error_long(git_reference_create(&local_ref, lua_repo->repo, name,
                                        &(lua_oid->oid), force, log_message),
                   "Error creating reference with details provided", NULL);
...
int lua_git_reference_create_matching(lua_State *L) {
  if (lua_gettop(L) != 6) {
    return luaL_error(
        L,
        "expecting exactly 6 argument : luagit2_repository,name,luagit2_oid(target),\
  ...
  const luagit2_oid *lua_oid =
      (luagit2_oid *)luaL_checkudata(L, 3, "luagit2_oid");
  const int force = luaL_checkinteger(L, 4);
  const luagit2_oid *current_oid =
      (luagit2_oid *)luaL_checkudata(L, 5, "luagit2_oid");
  ...
  git_reference *local_ref;
  check_error_long(git_reference_create_matching(
                       &local_ref, lua_repo->repo, name, &(lua_oid->oid), force,
                       &(current_oid->oid), log_message),
                   "Error creating matching reference", NULL);
  lua_ref->reference = local_ref;
...
int lua_git_reference_dup(lua_State *L) {
  ...
  luagit2_reference *lua_ref;
  const luagit2_reference *lua_ref_source =
      (luagit2_reference *)luaL_checkudata(L, 1, "luagit2_reference");
...
int lua_git_reference_dwim(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument : luagit2_repository,shorthand");
  }
  ...
  check_error_long(git_reference_dwim(&local_ref, lua_repo->repo, shorthand),
                   "Unable to Lookup a reference by DWIMing its short name",
                   NULL);
  lua_ref->reference = local_ref;
...
int lua_git_reference_ensure_log(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument : luagit2_repository,refname");
  }
...
int lua_git_reference_is_branch(lua_State *L) {
  ...
  const luagit2_reference *lua_ref =
      (luagit2_reference *)luaL_checkudata(L, 1, "luagit2_reference");
...
int lua_git_reference_iterator_glob_new(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument : luagit2_repository,glob_name");
  }
  ...
  lua_ref_iterator = (luagit2_reference_iterator *)lua_newuserdata(
      L, sizeof(*lua_ref_iterator));
  ...
  check_error_long(git_reference_iterator_glob_new(&local_ref_iterator,
                                                   lua_repo->repo, glob),
...
int lua_git_reference_lookup(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(L,
                      "expecting exactly 2 argument : luagit2_repository,name");
  }
...
int lua_git_reference_next(lua_State *L) {
  if (lua_gettop(L) != 1) {
    return luaL_error(
        L, "expecting exactly 1 argument : luagit2_reference_iterator");
  }

  luagit2_reference *lua_ref;
  const luagit2_reference_iterator *lua_ref_iterator =
      (luagit2_reference_iterator *)luaL_checkudata(
          L, 1, "luagit2_reference_iterator");
...
int lua_git_reference_peel(lua_State *L) {
  if (lua_gettop(L) != 2) {
    return luaL_error(
        L, "expecting exactly 2 argument : luagit2_reference,luagit2_otype");
  }

  luagit2_object *lua_git_obj;
  const luagit2_reference *lua_ref =
      (luagit2_reference *)luaL_checkudata(L, 1, "luagit2_reference");
  const luagit2_otype *type =
      (luagit2_otype *)luaL_checkudata(L, 2, "luagit2_otype");
...
int lua_git_reference_remove(lua_State *L) {
  ...
  check_error_long(git_reference_remove(lua_repo->repo, name),
                   "Unable to remove the named reference from the repository",
                   NULL);
  return 1;
}
...
int lua_git_reference_rename(lua_State *L) {
  if (lua_gettop(L) != 4) {
    return luaL_error(L,
                      "expecting exactly 4 argument : "
                      "luagit2_reference,new_name,force(int),log_message");
  }

  luagit2_reference *lua_ref;
  const luagit2_reference *lua_ref_old =
      (luagit2_reference *)luaL_checkudata(L, 1, "luagit2_reference");
  ...
  git_reference *local_ref;
  check_error_long(git_reference_rename(&local_ref, lua_ref_old->reference,
                                        new_name, force, log_message),
...
int lua_git_reference_set_target(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(L,
                      "expecting exactly 3 argument : "
                      "luagit2_reference,luagit2_oid,log_message");
  }
  ...
  const luagit2_reference *lua_ref_old =
      (luagit2_reference *)luaL_checkudata(L, 1, "luagit2_reference");
  const luagit2_oid *lua_oid =
      (luagit2_oid *)luaL_checkudata(L, 2, "luagit2_oid");
  ...
  git_reference *local_ref;
  check_error_long(git_reference_set_target(&local_ref, lua_ref_old->reference,
                                            &(lua_oid->oid), log_message),
...
int lua_git_reference_symbolic_create(lua_State *L) {
  if (lua_gettop(L) != 5) {
    return luaL_error(
        L,
        "expecting exactly 5 argument : luagit2_repository,name,target,\
			force(int),log_message");
  }
  ...
  git_reference *local_ref;
  check_error_long(
      git_reference_symbolic_create(&local_ref, lua_repo->repo, name, target,
                                    force, log_message),
...
int lua_git_reference_symbolic_create_matching(lua_State *L) {
  if (lua_gettop(L) != 6) {
    return luaL_error(
        L,
        "expecting exactly 6 argument : luagit2_repository,name,target,\
  ...
  git_reference *local_ref;
  check_error_long(git_reference_symbolic_create_matching(
                       &local_ref, lua_repo->repo, name, target, force,
                       current_value, log_message),
                   "Unable to create matching symbolic reference", NULL);
  lua_ref->reference = local_ref;
...
int lua_git_reference_symbolic_set_target(lua_State *L) {
  if (lua_gettop(L) != 3) {
    return luaL_error(
        L,
        "expecting exactly 3 argument : luagit2_reference,target,log_message");
  }
  ...
  git_reference *local_ref;
  check_error_long(git_reference_symbolic_set_target(
                       &local_ref, lua_ref_old->reference, target, log_message),
...
int lua_git_reference_symbolic_target(lua_State *L) {
  ...
  const luagit2_reference *lua_ref =
      (luagit2_reference *)luaL_checkudata(L, 1, "luagit2_reference");
  const char *target_symbolic =
      git_reference_symbolic_target(lua_ref->reference);
