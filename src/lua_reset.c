...
int lua_git_reset(lua_State *L) {
  ...
  if ((reset_t = (strcasecmp(option, "mixed") == 0 ? GIT_RESET_MIXED : 0)) !=
      0) {
    goto set_val;
  }
...
set_val : {
  check_error_long(git_reset(Repository->repo, lua_obj->object, reset_t,
                             &local_checkout_opts),
